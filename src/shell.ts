...
import { MicrosandboxBackend, resolveGuestWorkdir, type MicrosandboxConfig, type MicrosandboxInternals } from './backend.ts'
...
function requirePolicy(policy: SandboxExecutionPolicy | undefined): SandboxExecutionPolicy {
  if (policy === undefined) {
...
export function buildShellConfinement(
  spec: Pick<ShellExecSpec, 'command' | 'workdir' | 'env' | 'dshEnv'>,
  ...
  const confined = backend.confine(
    ['bash', '-c', spec.command],
    { ...policy, mode },
    { workdir: guestWorkdir, env },
  )
  return {
    ...confined,
...
export class MicrosandboxShellExecutor extends LocalBashExecutor {
  ...
  static Config: z<LocalConfig> = z.object({
    cwd: z.string(),
