...
- **Execution-time runner failures are classified** with `runnerFailureRules` calibrated against real msb 0.6.15 stderr (image pull, mount path, sandbox start, and invalid-config errors).
...
- Node `^22.19.0 || >=24.0.0`
- `microsandbox` 0.6.15+ (installed as a dependency)
